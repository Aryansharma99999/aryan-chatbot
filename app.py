...
if is_admin:
    ...
    uploaded = st.sidebar.file_uploader("Upload images (jpg, png, webp)", type=["png","jpg","jpeg","webp"], accept_multiple_files=True)
    if uploaded:
        for file in uploaded:
            filename = f"{datetime.utcnow().strftime('%Y%m%d%H%M%S')}_{file.name}"
            with open(GALLERY_DIR / filename, "wb") as f:
                f.write(file.getbuffer())
        st.sidebar.success("Saved uploaded file(s).")
        st.experimental_rerun()
    st.sidebar.markdown("---")
    st.sidebar.subheader("Existing images")
    imgs = sorted(GALLERY_DIR.glob("*"), key=os.path.getmtime, reverse=True)
    if not imgs:
        st.sidebar.info("No images yet. Upload using the uploader above.")
    else:
        for img in imgs:
...
# ---------------------------
# Build gallery HTML from files
# ---------------------------
def get_gallery_html():
    imgs = sorted(GALLERY_DIR.glob("*"), key=os.path.getmtime, reverse=True)
    if not imgs:
        return "<div class='card'>No photos yet — upload via ?admin=1</div>"
    html = ""
    for img in imgs:
        # reference relative path so Streamlit serves it
        html += f\"\"\"
        <div class="card">
...
# ---------------------------
# Full HTML template (injected into components.html)
# ---------------------------
html_template = """
<!doctype html>
<html>
<head>
  ...
  <style>
    ...
    body{background:linear-gradient(180deg,#0b0020,#210034 60%);overflow-x:hidden;}
...
<script>
  ...
  sendBtn.addEventListener('click', ()=> {
    ...
    setTimeout(()=> { const bot = document.createElement('div'); bot.className = 'bot-msg'; bot.textContent = \"Thanks — here's a sample reply about Aryan. (Replace with your real assistant integration.)\"; chatBody.appendChild(bot); chatBody.scrollTop = chatBody.scrollHeight; }, 700);
  });
  ...
  function typeDelete(element, text, speed=40){
    return new Promise((resolve)=> {
      ...
      const t = setInterval(()=>{
        ...
        i++;
        if(i >= text.length){ clearInterval(t); setTimeout(()=> { resolve(); }, 900); }
      }, speed);
    }).then(()=> {
