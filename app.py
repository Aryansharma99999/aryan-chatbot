...
if is_admin:
    ...
    if uploaded:
        for file in uploaded:
            ...
            with open(GALLERY_DIR / filename, "wb") as f: f.write(file.getbuffer())
        st.sidebar.success("Uploaded Successfully."); st.experimental_rerun()
    st.sidebar.markdown("---")
    st.sidebar.subheader("Existing Images")
    imgs = sorted(GALLERY_DIR.glob("*"), key=os.path.getmtime, reverse=True)
    for img in imgs:
        c1, c2 = st.sidebar.columns([3,1]); c1.write(img.name)
        if c2.button("Delete", key=f"del_{img.name}"):
            img.unlink(); st.sidebar.success("Deleted."); st.experimental_rerun()
...
.stars{background:transparent url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="2" height="2"><circle cx="1" cy="1" r="1" fill="white" fill-opacity="0.03"/></svg>') repeat;background-size:3px 3px;animation:starMove 120s linear infinite;}
...
/* SECTIONS */
.section{padding:40px 60px;} .grid{display:grid;gap:20px;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));}
...
function sendChat(){
  ...
  var u = document.createElement('div'); u.className = 'user-msg'; u.textContent = val; body.appendChild(u);
  input.value = '';
  setTimeout(function(){
    var b = document.createElement('div'); b.className = 'bot-msg'; b.textContent = "Thanks — here's a sample reply about Aryan. Replace with your assistant integration.";
    body.appendChild(b); body.scrollTop = body.scrollHeight;
  },700);
  body.scrollTop = body.scrollHeight;
}

// Typing utilities (type + delete)
async function typeEffect(elem, text, speed){
  elem.innerHTML = '';
  for(let i=0;i<text.length;i++){ elem.innerHTML += text.charAt(i); await new Promise(r=>setTimeout(r, speed)); }
}
async function deleteEffect(elem, speed){
  while(elem.innerHTML.length>0){ elem.innerHTML = elem.innerHTML.slice(0,-1); await new Promise(r=>setTimeout(r, speed)); }
}
...
async function startLoop(){
  var main = document.getElementById('typingLine');
  var about = document.getElementById('aboutMini');
  while(true){
    for(let r of roles){ await typeEffect(main, r, 40); await new Promise(r=>setTimeout(r,900)); await deleteEffect(main, 22); }
    for(let l of aboutLines){ await typeEffect(main, l, 32); about.innerHTML = l; await new Promise(r=>setTimeout(r,1600)); await deleteEffect(main, 22); }
  }
}
...
# ----------------------------------------------------------------
# Insert gallery HTML into the template and render
# ----------------------------------------------------------------
html_template = html_template.replace("###GALLERY###", gallery_html)

# render full page — choose a height that fills the viewport; scrolling False so page fills window
